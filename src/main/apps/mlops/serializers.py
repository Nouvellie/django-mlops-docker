...
class FashionMnistSerializer(serializers.Serializer):
	...
	def validate(self, attrs: Generic[API_INPUTS]) -> Generic[API_INPUTS]:
		...
		if not media_type.endswith("png"):
			raise CustomError(detail="Only .png format will be accepted.", code=415)
		...
		if attrs.get('file'):
			...
			if not media_type.endswith(('docx', 'txt', 'md')):
				if not attrs.get('text'):	
					raise CustomError(detail="Only '.doc .txt or .md' format will be accepted.", code=415)
				else:
			...
			raise CustomError(detail="You must send at least one. (text or file)", code=400)
		return super().validate(attrs)
...
class CatsVsDogsSerializer(serializers.Serializer):
	...
	def validate(self, attrs: Generic[API_INPUTS]) -> Generic[API_INPUTS]:
		...
		if not media_type.endswith("jpg"):
			raise CustomError(detail="Only .jpg format will be accepted.", code=415)
		return super().validate(attrs)